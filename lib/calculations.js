 ...
 * Supports both percentage-based and direct area input modes
 */
export function calculateAreas(totalArea, percentRacking, mezzPercent, useDirectAreaInput = false, hdRackArea = 0, mezzanineArea = 0) {
  let rackingArea, mezzArea, hdArea, stagingArea;
  
  if (useDirectAreaInput) {
    // Direct area input mode
    hdArea = hdRackArea || 0;
    mezzArea = mezzanineArea || 0;
    rackingArea = hdArea + mezzArea;
    stagingArea = totalArea - rackingArea;
    
    // Ensure staging area doesn't go negative
    if (stagingArea < 0) {
      stagingArea = 0;
    }
  } else {
    // Percentage-based mode (original behavior)
    rackingArea = totalArea * (percentRacking / 100);
    mezzArea = rackingArea * (mezzPercent / 100);
    hdArea = rackingArea - mezzArea;
    stagingArea = totalArea - rackingArea;
  }
 ...
 */
export function calculateMezzanineCBM(mezzArea, mezzClearHeight, mezzLevels = 1) {
  const mezzTotalCuft = mezzArea * mezzClearHeight * mezzLevels;
...
export function validateInputs(inputs, calculations) {
  ...
  // Check area overflow
  const { rackingArea, totalArea, useDirectAreaInput } = calculations.areas;
  if (rackingArea > totalArea) {
    if (useDirectAreaInput) {
      errors.push({
        field: 'hdRackArea',
        message: `Total racking area (${rackingArea.toLocaleString()} sq.ft) exceeds warehouse area (${totalArea.toLocaleString()} sq.ft). Reduce HD Rack or Mezzanine area.`
      });
    } else {
      errors.push({
...
export function calculateWarehouseCapacity(inputs) {
  ...
  const areas = calculateAreas(totalArea, percentRacking, mezzPercent, useDirectAreaInput, hdRackArea, mezzanineArea);
  ...
  const mezzCBM = calculateMezzanineCBM(areas.mezzArea, mezzClearHeight, mezzLevels || 1);
